# plots various plots for neutrophils
...
colset.attr <- brewer.pal(n = 4, name = "Greys")
## input
load(_data_here) # ** require input

## heatmap
neutro.mat <- assay(vsdD[which(rownames(vsdD)%in%geneSets$Hist_Neutro_quant), ]) ## expression data
annotMat <- data.frame(diagnosis=colData(vsdD)$newNames) ## diagnosis annotation
annotMat$neutrophils <- dataMat$Hist_Neutro_quant ## add clinical phenotype data
rownames(annotMat) <- colnames(neutro.mat)
names(colset) <- levels(annotMat$diagnosis)
names(colset.attr) <- levels(annotMat$neutrophils)
colset.list <- list(diagnosis = colset, neutrophils=colset.attr)
## plotting
pheatmap(neutro.mat, color = blue2yellow(100), scale = "row", annotation_col = annotMat, fontsize_row = 5, annotation_colors = colset.list, cutree_rows = 2, cutree_cols = 2)

## circular barplot 
plotMat <- data.frame(figdiag=annotMat$diagnosis, neutro=dataMat$Hist_Neutro_quant, id=1:nrow(annotMat))
plotMat$neutro <- as.numeric(as.matrix(plotMat$neutro))
...
p <- ggplot(plotMat, aes(x=as.factor(id), y=neutro, fill=figdiag)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  
  geom_bar(aes(x=as.factor(id), y=neutro, fill=figdiag), stat="identity", alpha=0.5) +
  # customize colors
...
## GO processes
neutro.pathnet <- do.call(rbind, lapply(1:nrow(neutro.path), FUN=function(i) {
  x<- neutro.path$symbols[i]; 
  data.frame(genes=gsub("\\s", "", unlist(strsplit(unlist(x), ","))), 
  GO_Term =neutro.path$Var1[i])}))

neutro.path$Var1 <- factor(neutro.path$Var1, levels = neutro.path$Var1[order(neutro.path$value)])

p <- ggplot(data=neutro.path, aes(x=Var1, y=value)) +
